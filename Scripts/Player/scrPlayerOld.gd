...
func _physics_process(delta):
	...
	if !frozen:
		...
		if !is_walljumping:
			...
			# Water checks for constant collisions. We only do when we're
			# actually inside of water
			if in_water:
				handle_water()
...
func handle_jumping() -> void:
	...
	if Input.is_action_just_pressed("button_jump"):
		if (is_on_floor() == true) or (inside_platform_jump):
			velocity.y = -s_jump_speed
			GLOBAL_SOUNDS.play_sound("sndJump")
			
			# Gives d_jump if jumping inside a platform, similar to what gamemaker engines do
		...
		elif (d_jump) or (in_water and !is_catharsis_water) or (GLOBAL_GAME.debug_inf_jump):
...
func _on_platforms_body_exited(body):
	if !$extraCollisions/Platforms.has_overlapping_bodies():
		...
		# lots of precision!.
		# Checks if a "snap" variable exists first
		if "snap" in body:
			if body.snap:
				
				# Margin variables. Makes platforms feel more "natural" and tight
				var horizontal_margin = 22 * body.get_scale().x
				var vertical_margin = 16
				
				# Horizontal and vertical detection
				if (global_position.x > body.global_position.x - horizontal_margin) and (global_position.x < body.global_position.x + horizontal_margin):
					if body.global_position.y - vertical_margin > global_position.y:
						
						# Sets the player velocity to a very high value one
						# physics frame after it leaves the platform, assuming
						# the player is still moving up.
						# Brute force approach that somehow works well enough,
						# taking advantage of Godot's physics
